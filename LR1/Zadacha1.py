...
class EPRCalculator:
    ...
    def bn(self, n, k, r):
        return ((k * r * spherical_jn(n - 1, k * r) - n * spherical_jn(n, k * r)) /
                (k * r * self.hankel_three_kind(n - 1, k, r) - n * self.hankel_three_kind(n, k, r)))
...
class EPRPlotter:
    @staticmethod
    def save_to_json(frequencies, sigmas: list, filename="output.json"):
        data = {
            "data": []
        }
        with open(filename, "w") as file:
            for i in range(len(sigmas)):
                data["data"].append({"freq": frequencies[i], "lambda": 3 * 10 ** 8 / frequencies[i], "rcs": sigmas[i]})
...
if __name__ == '__main__':
    ...
    EPRPlotter.save_to_json(frequencies, sigmas, filename=output_json)
    ...
    plt.grid(True)
    plt.savefig('task1var13.png', bbox_inches='tight')
    plt.show()